...
# TODO 他のスクリプトと共通化する
def generate_step_logs(args: argparse.Namespace, current_loss, avr_loss,
...
def train(args):
    ...
    if args.dataset_class is None:
        ...
        if use_user_config:
            ...
            if use_dreambooth_method:
                ...
                user_config = {
                    "datasets": [{"subsets": [{"image_dir": args.train_data_dir,
    ...
    args.optimizer_args = ["weight_decay=1e-1", ]
    args.training_comment = "trigger word is agnes tachyon \\(umamusume\\)"
    #args.network_args = ["conv_rank=64", "conv_alpha=64", "algo=locon", ]
    ...
    parent, folder_name = os.path.split(args.output_dir)
    ...
    ##################################################################################################################
    # 4) loss bowl for log save
    record_base_dir = os.path.join(args.output_dir, 'record')
            ...
            with accelerator.accumulate(network):
                ...
                if accelerator.is_main_process:
                    ...
                    for (layer_name, param), param_dict in zip(network.named_parameters(), optimizer.param_groups):
                        ...
                        try:
                            grad_norm = param_dict['params'][0].grad.data.norm(2).item()
                            gradient_dict[layer_name].append(grad_norm)
                        except:
    ...
    effective_layers, un_effective_layers = [], []
    for layer in gradient_dict.keys():
        avg_grad_norm = sum(gradient_dict[layer]) / len(gradient_dict[layer])
        if 'down' in layer :
            e_layer = layer.split('.lora_down')[0]
            if avg_grad_norm < 0.001 :
                un_effective_layers.append(e_layer)
            else :
                effective_layers.append(e_layer)


    print('effective_layers : ', effective_layers)
    effective_layer_dir = os.path.join(record_base_dir, 'effective_layers.txt')
    with open(effective_layer_dir, 'w') as f:
        for layer in effective_layers:
            f.write(layer + '\n')
    un_effective_layer_dir = os.path.join(record_base_dir, 'un_effective_layers.txt')
    with open(un_effective_layer_dir, 'w') as f:
        for layer in un_effective_layers:
            f.write(layer + '\n')
    del network
    # ----------------------------------------------------------------------------------------------------------------- #
    # reload model
    ...
    # 後方互換性を確保するよ
    try:
        print('calculate trainable params')
        ...
        trainable_params = network.prepare_optimizer_params(args.text_encoder_lr, args.unet_lr)
    all_params = []
    for i, trainable_param in enumerate(trainable_params) :
        print(f'{i}. trainable param')
        lr = trainable_param["lr"]
        params = trainable_param["params"]
        for param in params:
            param_dict = {"lr": lr, "params": param}
            all_params.append(param_dict)
    print(f'len of all params : {len(all_params)} ...... ')
    optimizer_name, optimizer_args, optimizer = train_util.get_optimizer(args, all_params)
    lr_scheduler = train_util.get_scheduler_fix(args, optimizer, accelerator.num_processes)
    if args.full_fp16:
        assert (
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                train_util.sample_images(accelerator, args, None, global_step, accelerator.device, vae, tokenizer,
                                         text_encoder, unet)
    ...
    gradient_dict_save_dir = os.path.join(record_base_dir, 'gradient_norm.pickle')
    with open(gradient_dict_save_dir, 'wb') as fw:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cache_latents_to_disk",
                        action="store_true",
    ...
    parser.add_argument("--save_last_n_steps",type=int,
                        default=None,
    ...
    parser.add_argument("--output_config", action="store_true", help="output command line args to given .toml file / 引数を.tomlファイルに出力する")
    ...
    parser.add_argument("--use_8bit_adam", action="store_true", help="use 8bit AdamW optimizer (requires bitsandbytes) / 8bit Adamオプティマイザを使う（bitsandbytesのインストールが必要）",)
    parser.add_argument("--use_lion_optimizer",action="store_true",help="use Lion optimizer (requires lion-pytorch) / Lionオプティマイザを使う（ lion-pytorch のインストールが必要）",)
    parser.add_argument("--learning_rate", type=float, default=2.0e-6, help="learning rate / 学習率")
    parser.add_argument("--max_grad_norm", default=1.0, type=float, help="Max gradient norm, 0 for no clipping / 勾配正規化の最大norm、0でclippingを行わない")
    ...
    parser.add_argument("--network_train_text_encoder_only", action="store_true",help="only training Text Encoder part / Text Encoder関連部分のみ学習する")
    parser.add_argument("--training_comment", type=str, default=None,help="arbitrary comment string stored in metadata / メタデータに記録する任意のコメント文字列")
    ...
    parser.add_argument("--search_epochs", type = int, default = 1)
