...
"""
def create_network_text_self_attn_only_training(
    multiplier: float,
    network_dim: Optional[int],
    network_alpha: Optional[float],
    vae: AutoencoderKL,
    ...
    **kwargs,
    ...
    network = LoRANetworktext_self_attn_only_training(
        text_encoder,
        unet,
        multiplier=multiplier,
        lora_dim=network_dim,
...
def create_network_from_weights(multiplier, file, block_wise, vae, text_encoder, unet, weights_sd=None,
...
class LoRANetworktext_self_attn_only_training(torch.nn.Module):
                    ...
                    for child_name, child_module in module.named_modules():
                        ...
                        if is_linear or is_conv2d:
                            ...
                            if not is_unet :
                                ...
                                loras.append(lora)
                            if is_unet :
                                if 'attn2_to_k' in lora_name or 'attn2_to_v' in lora_name or 'attn1' in lora_name :
                                    lora = module_class(lora_name,
                                                        child_module,
                                                        self.multiplier,
                                                        dim,
